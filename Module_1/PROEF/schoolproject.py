...
def GoToVraag(number):
    ...
    if number == 1:
        ...
        vraag = "Na het eten bedank je de man.\nJe krijgt een kaart van de man.\nJe gaat op pad.\nJe ziet een in de verte man lopen."
        opties = ["Je loopt naar de man in de verte.", "Je gaat de andere kant op.", "Je zoekt door naar de uitgang van het bos."]
        UserAntwoord = Vraag(vraag, opties)
        if UserAntwoord in ["1", "a"]:
            GoToVraag(13)
        if UserAntwoord in ["2", "b"]:
            Screen_Death("Je raakt verdwaalt.\nJe ben overvallen door zwervers.\nOvergereden door 2 fietsers.\nGeslagen door een muis.\nEn toen overleden door liefdesverdriet want niemand houd van jou.")
        if UserAntwoord in ["3", "c"]:
           Screen_Death("Je zocht voor uren.\nEn je vroor dood.")

    elif number == 13:
        vraag = "De man blijkt een boswachter te zijn.\nHij vraagt om je leeftijd in hele jaren.\nHoe oud ben je?"
        print(vraag)
        leeftijd = input()
        try:
            ...
            tprint("GAME OVER")
            print(f"     {leeftijd} is geen getal zegt de boswachter.\nHij houd niet zo van grapjes.\nEn heeft je gearesteerd")
            print()
            input("Druk op enter om opnieuw te beginnen")
            clear_screen()
            slowprint(intro)
        ...
        slowprint('''Wat doe je helemaal alleen in het bos?
Ik ben verdwaalt.
Ik zal je wel de weg naar de bewoonde wereld laten zien
*Je loopt samen met de boswachter het bos uit...''')
        time.sleep(2)
        clear_screen()
        tprint("YOU WIN")
        print("Je bent uit het bos gekomen")
    elif number == 15:
        vraag = "Wat doe je helemaal alleen in het bos?"
        opties = ["Ik ben verdwaalt.", "Ik ben aan het wandelen."]
        UserAntwoord = Vraag(vraag, opties)
        if UserAntwoord in ["1", "a"]:
            GoToVraag(13)
        if UserAntwoord in ["2", "b"]:
